...
## Custom Streak Messages

The bot loads win/lose streak alert text from `streak_messages.json`.
If the file does not exist, the bot creates it with default messages the first time it runs.

To customize streak alerts:
1. Open `streak_messages.json`.
2. Edit the message templates for `win_streak`, `lose_streak`, and `streak_break`.
3. Keep placeholders like `{streak}` if you want the streak number inserted.
4. Save the file and restart the bot.

Supported keys:
- exact streak values like `3`, `4`, `5`, etc.
- threshold values like `12+` or `20+` to match any streak greater than or equal to that number

Example:
```json
"win_streak": {
  "3": "🔥 ON FIRE (3W) awoo awoo awoo",
  "20+": "🔥 {streak} WINS SEGUIDAS (LEYENDA) WIDESEXO"
}
```
...
- `streak_messages.json` - Editable streak notification templates
- `bot.log` - Bot logs
